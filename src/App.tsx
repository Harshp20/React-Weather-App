import { useEffect, useState } from 'react'
import axios from 'axios'
import WeatherUI from './components/WeatherUI/WeatherUI';
import './App.css';
import { Weather } from './components/WeatherUI/WeatherUI'
import SearchInput from './components/SearchInput/SearchInput';

function App() {
  const [weatherData, setWeatherData] = useState({} as Weather)
  const city_Name = 'Indore'

  useEffect(() => {
    axios.get(`https://api.openweathermap.org/data/2.5/weather?q=${city_Name}&appid=${process.env.REACT_APP_API_KEY}&units=metric`)
      .then(({ data }) => {
        console.log(data)
        const [{ main: weather_mood }] = data.weather
        const { temp: temperature, feels_like, temp_min, temp_max, pressure, humidity } = data.main
        const visibility = data.visibility
        const { speed: wind_speed, wind_direction_deg } = data.wind
        const { country } = data.sys
        const { name: city } = data

        const weatherInfo = {
          weather_mood,
          temperature,
          feels_like,
          temp_min,
          temp_max,
          pressure,
          humidity,
          visibility,
          wind_speed,
          wind_direction_deg,
          country,
          city
        }

        setWeatherData(weatherInfo)
      }
      )
  }, [])

  return (
    <>
      <SearchInput />
      <WeatherUI weatherData={weatherData} />
      <div style={{ color: 'white' }}>
        <pre>
          {JSON.stringify(weatherData, null, 2)}
        </pre>
      </div>
    </>
  );
}

export default App;
